// SCOP status deck (3 slides: QuickBase Status, Pathwave SCOP, Overall SCOP
// Status). Faithful port of docs/scop/build_scop_deck.js — every layout
// number, color, and ordering choice was iterated on with CJ and approved.
// This only renders; it consumes the view objects from app/lib/scop.ts and
// recomputes nothing.
//
// Runs client-side: pptxgenjs writeFile() triggers a browser download, same
// as the Decom slides on the Reports page.

import pptxgen from 'pptxgenjs'
import type { QuickBaseView, PathwaveView, OverallView } from './scop'
...
function kpiCard(
  slide: pptxgen.Slide, x: number, y: number, w: number, h: number,
...
function footer(slide: pptxgen.Slide, text: string) {
...
export interface ScopDeckData {
  totalNokiaHops: number
  constructionComplete: number
  notYetConstructed: number
  qbView: QuickBaseView
  pwView: PathwaveView
...
export async function buildScopDeck(data: ScopDeckData, outputPath: string): Promise<string> {
  ...
  // ── Slide 2: Pathwave SCOP — Contractor Action Items ──────────────────────
  ...
  const gcTableRows: pptxgen.TableRow[] = [
    [
  ...
  const oadTableRows: pptxgen.TableRow[] = [
    [
  ...
  ;(pwView.oadSites || []).forEach((site, i) => {
    const rowFill = i % 2 === 0 ? WHITE : 'EEF2FA'
    oadTableRows.push([
      { text: site.hop, options: { fontSize: 8.5, fill: { color: rowFill }, color: DARK } },
      { text: site.gc ?? '', options: { fontSize: 8.5, fill: { color: rowFill }, color: DARK } },
      { text: site.note, options: { fontSize: 8.5, fill: { color: rowFill }, color: DARK } },
    ])
  })
