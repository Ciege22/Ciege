...
export default function GCCallPage() {
  ...
  const processRows = useCallback((rows: unknown[][], _filename: string) => {
    ...
    const ntpOwnCol  = headers.findIndex(h => String(h).trim().replace(/^'+|'+$/g, '') === 'NTP Action Owner')
    const ntpWaitCol = headers.findIndex(h => String(h).trim().replace(/^'+|'+$/g, '') === 'NTP is waiting on')
          ...
          <div className="mb-6 bg-yellow-950 border border-yellow-600 rounded-xl p-5">
            ...
            <div className="mt-4 flex gap-3">
              <button onClick={async () => {
                ...
                setPmUpdates(r)
                await supabase.from('pm_updates_cache').upsert({ id: 'gc-view', updates: JSON.stringify(r), updated_at: new Date().toISOString() })
              ...
              <button onClick={async () => {
                ...
                await supabase.from('pm_updates_cache').upsert({ id: 'gc-view', updates: JSON.stringify([]), updated_at: new Date().toISOString() })
              }}
